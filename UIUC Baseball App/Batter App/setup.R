...
# Username = U Illinois
# Password (email danielthom18@outlook.com for password)
...

{
  ...
  # Calculate Plate Appearances in a Game
  
  Batter_Arrange <- Data |>
    filter(BatterTeam == "ILL_ILL") |>
    arrange(batter_name, GameUID, PAofInning, inning) |>
    mutate(Scenario = paste(batter_name, GameUID)) |>
    mutate(plate_app = paste(PAofInning, inning))
  
  Batter_List <- split(Batter_Arrange, Batter_Arrange$Scenario)
  
  Batter_List_2 <- lapply(Batter_List, function(x) {
    df = summarise(x, .by = "plate_app", n = n())
    further_df = mutate(df, BatterPA = 1:nrow(df))
    left_join(x, further_df, by = "plate_app")
  })
  ...
  PA_Seq <- Batter_Data |>
    arrange(batter_name, GameUID, inning, PAofInning, pitch_number) |>
  ...
  Batter_Data <- bind_rows(Seq_List_2)
  ...
  Batter_Data <- Batter_Data |>
    filter(!is.na(release_speed) & !is.na(VertRelAngle) & !is.na(HorzRelAngle) & 
             ...
             !is.na(effective_speed) & !is.na(SpeedDrop)) |>
    mutate(count_dash = "-") |>
