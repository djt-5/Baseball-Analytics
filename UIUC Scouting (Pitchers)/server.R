server <- function(input, output) {
  ...
  pitch_kzone_slg <- reactive({
    ...
    Pitcher_Filtered() |>
      ...
      filter(VertBreak >= input$kzone_break_y[1] &
               VertBreak <= input$kzone_break_y[2]) |>
      filter(events != "Undefined") |>
      filter(events != "Sacrifice") |>
      filter(events != "Walk") |>
      filter(description != "HitByPitch") |>
      summarize(.by = c("width","height"), 
                `SLG` = mean(slugging)) |>
  ...
  pitch_timeline_exit_velo <- reactive({
    ...
    Pitcher_Filtered() |>
      ...
      select(pitcher_name, release_speed, `Pitches Thrown`) |>
      summarise(.by = "Pitches Thrown", `Mean Exit Velo` = mean(release_speed)) |>
  ...
  Pitch_Filter_Table <- reactive({
    ...
    OBP <- Pitcher_Data |>
      filter(events != "Undefined" & !is.na(events)) |>
      summarise(.by = "pitch_name", `On Base %`  = mean(on_base))
    
    SLG <- Pitcher_Data |>
      filter(events != "Undefined" & events != "Walk" &
               events != "Sacrifice" & description != "HitByPitch" &
               !is.na(events)) |>
