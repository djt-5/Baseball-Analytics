{
  ...
  Flyouts <- Flyouts |>
    mutate(predicted_state = ifelse(base_out_state == "0 1 0 0" & prediction == "Yes",
                                    "0 0 1 1", NA)) |>
    mutate(predicted_state = ifelse(base_out_state == "0 1 0 1" & prediction == "Yes",
                                    "0 0 1 2", predicted_state)) |>
    mutate(predicted_state = ifelse(base_out_state == "0 1 0 0" & prediction == "No",
                                    "0 0 0 2", predicted_state)) |>
    mutate(predicted_state = ifelse(base_out_state == "0 1 0 1" & prediction == "No",
  ...
  ThirdBase <- rbind(Flyouts, Singles, Doubles, Triples, Force_Outs)
  
  write_csv(ThirdBase, "ThirdBase.csv")
}
