...

{
  ...
  Data <- Data |>
    ...
    mutate(safe = ifelse(tag_attempt == 1 & grepl("scores", des), 1, NA)) |>
    mutate(safe = ifelse(tag_attempt == 1 & grepl("out at home", des), 0, safe)) |>
    select(safe, tag_attempt, dist_from_base, arm_overall, sprint_speed)
  
  write_csv(Data, "DataHP.csv")
