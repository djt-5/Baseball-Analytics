server <- function(input, output) {
  ...
  App_Data <- reactive({
    ...
    if(input$previous_pitch != "Any") {
      Data <- Data |>
        ...
        filter(releasetotunnelratio >= input$releasetotunnelratio[1] & 
                 releasetotunnelratio <= input$releasetotunnelratio[2]) 
    }
  ...
  Pitch_Filter_Table <- reactive({
    ...
    Ground_Ball <- App_Data() |>
      summarise(.by = "pitch_name", `Ground Ball %` = mean(GroundBall, na.rm = TRUE))
    ...
    Tunnel_Difference <- App_Data() |>
      summarise(.by = "pitch_name", `Tunnel Difference (Ft)` =
                  mean(diffattunnel, na.rm = TRUE))
    
    Plate_Difference <- App_Data() |>
      summarise(.by = "pitch_name", `Plate Difference (Ft)` =
                  mean(diffatplate, na.rm = TRUE))
    
    Release_Difference <- App_Data() |>
      summarise(.by = "pitch_name", `Release Difference (Ft)` =
                  mean(diffatrelease, na.rm = TRUE))
    
    Release_To_Tunnel_Ratio <- App_Data() |>
  ...
  pitch_release_side <- reactive({
    ...
    App_Data() |>
      ...
      geom_point() +
      scale_y_continuous("Release Height (Feet)", limits = c(0,8)) +
      scale_x_continuous("Release Extension (Feet)", limits = c(4,8)) +
      geom_hline(yintercept = 0) +
      geom_vline(xintercept = 0) +
      theme_bw()
  })
  ...
  fastball_groundball <- reactive({
    
    App_Data() |>
      filter(!is.na(GroundBall)) |>
      rename(`Groundball On Next Pitch` = GroundBall) |>
      ggplot(aes(x = prev_plate_x, y = prev_plate_z, color = `Groundball On Next Pitch`, 
                 alpha = 0.5)) +
      geom_point() +
      scale_x_continuous("Width (Catcher POV)", limits = c(-2.5,2.5)) +
      scale_y_continuous("Height", limits = c(0,4.5)) +
  ...
  output$fastball_groundball <- renderPlot({fastball_groundball()}, height = 500)
  
  second_groundball <- reactive({
  ...
  output$second_groundball <- renderPlot({second_groundball()}, height = 500)
  ...
  plot_pfx <- reactive({
    if(input$response_select == "Whiff") {
      ...
      App_Data() |>
        filter(!is.na(pfx_x) & !is.na(pfx_z)) |>
        select(pitcher_name, pfx_x, pfx_z, GroundBall) |>
        ggplot(aes(x = pfx_x, y = pfx_z, z = GroundBall)) +
        stat_summary_hex(fun = "mean", bins = input$bins_break)
    } 
  })
  ...
  plot_posttunnelbreak <- reactive({
    if(input$response_select == "Whiff") {
      ...
      App_Data() |>
        filter(!is.na(post_tunnel_break_x) & !is.na(post_tunnel_break_z)) |>
        select(pitcher_name, post_tunnel_break_x, post_tunnel_break_z, GroundBall) |>
        ggplot(aes(x = post_tunnel_break_x, y = post_tunnel_break_z, z = GroundBall)) +
        stat_summary_hex(fun = "mean", bins = input$bins_ptb)
    } 
  })
  ...
  plot_release_speed <- reactive({
    if(input$response_select == "Whiff") {
      ...
      App_Data() |>
        filter(!is.na(release_speed) & !is.na(speeddiff)) |>
        select(pitcher_name, release_speed, speeddiff, GroundBall) |>
        ggplot(aes(x = release_speed, y = speeddiff, z = GroundBall)) +
        stat_summary_hex(fun = "mean", bins = input$bins_speed)
    } 
  })
  ...
  plot_release_spin <- reactive({
    if(input$response_select == "Whiff") {
      ...
      App_Data() |>
        filter(!is.na(release_spin_rate) & !is.na(spin_axis)) |>
        select(pitcher_name, release_spin_rate, spin_axis, GroundBall) |>
        ggplot(aes(x = release_spin_rate, y = spin_axis, z = GroundBall)) +
        stat_summary_hex(fun = "mean", bins = input$bins_spin)
    } 
  })
  ...
  plot_spin_eff <- reactive({
    if(input$response_select == "Whiff") {
      ...
      App_Data() |>
        ...
        summarise(.by = "spin_eff_cut",
                  N = n(),
                  GroundBall = mean(GroundBall)) |>
        ggplot(aes(x=spin_eff_cut, fill=GroundBall)) +
        geom_col(aes(y = N), width = 1)
    } 
  })
  ...
  plot_diffattunnel <- reactive({
    if(input$response_select == "Whiff") {
      App_Data() |>
        filter(!is.na(Whiff)) |>
        mutate(diffattunnel_cut = cut(diffattunnel,seq(0,6,input$bins_tunnel))) |>
        summarise(.by = "diffattunnel_cut",
                  N = n(),
                  Whiff = mean(Whiff)) |>
        ggplot(aes(x=diffattunnel_cut, fill=Whiff)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "Chase") {
      App_Data() |>
        filter(!is.na(Chase)) |>
        ...
        summarise(.by = "diffattunnel_cut",
                  N = n(),
                  Chase = mean(Chase)) |>
        ggplot(aes(x=diffattunnel_cut, fill=Chase)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "GroundBall") {
      App_Data() |>
        filter(!is.na(GroundBall)) |>
        ...
        summarise(.by = "diffattunnel_cut",
                  N = n(),
                  GroundBall = mean(GroundBall)) |>
        ggplot(aes(x=diffattunnel_cut, fill=GroundBall)) +
        geom_col(aes(y = N), width = 1)
    } 
  })
  
  output$plot_diffattunnel <- renderPlot({plot_diffattunnel()},height = 600, width = 1000)
  
  plot_diffatplate <- reactive({
    if(input$response_select == "Whiff") {
      App_Data() |>
        filter(!is.na(Whiff)) |>
        mutate(diffatplate_cut = cut(diffatplate,seq(0,6,input$bins_plate))) |>
        summarise(.by = "diffatplate_cut",
                  N = n(),
                  Whiff = mean(Whiff)) |>
        ggplot(aes(x=diffatplate_cut, fill=Whiff)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "Chase") {
      App_Data() |>
        filter(!is.na(Chase)) |>
        ...
        summarise(.by = "diffatplate_cut",
                  N = n(),
                  Chase = mean(Chase)) |>
        ggplot(aes(x=diffatplate_cut, fill=Chase)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "GroundBall") {
      App_Data() |>
        filter(!is.na(GroundBall)) |>
        ...
        summarise(.by = "diffatplate_cut",
                  N = n(),
                  GroundBall = mean(GroundBall)) |>
        ggplot(aes(x=diffatplate_cut, fill=GroundBall)) +
        geom_col(aes(y = N), width = 1)
    } 
  })
  
  output$plot_diffatplate <- renderPlot({plot_diffatplate()},height = 600, width = 1000)
  
  plot_diffatrelease <- reactive({
    if(input$response_select == "Whiff") {
      App_Data() |>
        filter(!is.na(Whiff)) |>
        mutate(diffatrelease_cut = cut(diffatrelease,seq(0,1,input$bins_release))) |>
        summarise(.by = "diffatrelease_cut",
                  N = n(),
                  Whiff = mean(Whiff)) |>
        ggplot(aes(x=diffatrelease_cut, fill=Whiff)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "Chase") {
      App_Data() |>
        filter(!is.na(Chase)) |>
        ...
        summarise(.by = "diffatrelease_cut",
                  N = n(),
                  Chase = mean(Chase)) |>
        ggplot(aes(x=diffatrelease_cut, fill=Chase)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "GroundBall") {
      App_Data() |>
        filter(!is.na(GroundBall)) |>
        ...
        summarise(.by = "diffatrelease_cut",
                  N = n(),
                  GroundBall = mean(GroundBall)) |>
        ggplot(aes(x=diffatrelease_cut, fill=GroundBall)) +
        geom_col(aes(y = N), width = 1)
    } 
  })
  
  output$plot_diffatrelease <- renderPlot({plot_diffatrelease()},height = 600, width = 1000)
  
  plot_releasetotunnelratio <- reactive({
    if(input$response_select == "Whiff") {
      App_Data() |>
        filter(!is.na(Whiff)) |>
        mutate(releasetotunnelratio_cut = cut(releasetotunnelratio,seq(0,2,input$bins_rttr))) |>
        summarise(.by = "releasetotunnelratio_cut",
                  N = n(),
                  Whiff = mean(Whiff)) |>
        ggplot(aes(x=releasetotunnelratio_cut, fill=Whiff)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "Chase") {
      App_Data() |>
        filter(!is.na(Chase)) |>
        ...
        summarise(.by = "releasetotunnelratio_cut",
                  N = n(),
                  Chase = mean(Chase)) |>
        ggplot(aes(x=releasetotunnelratio_cut, fill=Chase)) +
        geom_col(aes(y = N), width = 1)
    } else if(input$response_select == "GroundBall") {
      App_Data() |>
        filter(!is.na(GroundBall)) |>
        ...
        summarise(.by = "releasetotunnelratio_cut",
                  N = n(),
                  GroundBall = mean(GroundBall)) |>
        ggplot(aes(x=releasetotunnelratio_cut, fill=GroundBall)) +
        geom_col(aes(y = N), width = 1)
    } 
  })
  
  output$plot_releasetotunnelratio <- renderPlot({plot_releasetotunnelratio()},height = 600, width = 1000)
    ...
    if(input$response_select == "Whiff") {
      ...
      App_Data() |>
        ...
        summarise(.by = "breaktotunnelratio_cut",
                  N = n(),
                  GroundBall = mean(GroundBall)) |>
        ggplot(aes(x=breaktotunnelratio_cut, fill=GroundBall)) +
        geom_col(aes(y = N), width = 1)
    } 
  })
