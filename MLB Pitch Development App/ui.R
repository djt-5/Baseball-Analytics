{
  ...
  ui <- dashboardPage(skin = "blue", 
    dashboardHeader(title = "Pitch Development"),
    dashboardSidebar(
      ...
      tabItems(
        ...
        tabItem(tabName = "k_zone",
                tabBox(
                  ...
                  tabPanel("Ground Balls", plotOutput('fastball_groundball'))
                ),
                tabBox(
                  ...
                  tabPanel("Ground Balls", plotOutput('second_groundball'))
                )
        ),
        tabItem(tabName = "var_eff",
                tabBox(
                  ...
                  tabPanel("Tunnel Difference", 
                           sliderInput("bins_tunnel", "Bin Break:", min = 0.01, max = 0.2, value = 0.1, step = 0.01),
                           plotOutput('plot_diffattunnel')),
                  tabPanel("Plate Difference", 
                           sliderInput("bins_plate", "Bin Break:", min = 0.01, max = 0.2, value = 0.1, step = 0.01),
                           plotOutput('plot_diffatplate')),
                  tabPanel("Release Difference", 
                           sliderInput("bins_release", "Bin Break:", min = 0.01, max = 0.2, value = 0.1, step = 0.01),
                           plotOutput('plot_diffatrelease')),
                  tabPanel("Release to Tunnel Ratio", 
                           sliderInput("bins_rttr", "Bin Break:", min = 0.01, max = 0.2, value = 0.1, step = 0.01),
                           plotOutput('plot_releasetotunnelratio')),
                  tabPanel("Break to Tunnel Ratio", 
                           sliderInput("bins_bttr", "Bin Break:", min = 0.01, max = 0.2, value = 0.1, step = 0.01),
        ...
        tabItem(tabName = "filters",
                box(
                  ...
                  sliderInput('pfx_x', "Horizontal Break", min = -2.5, max = 2.5, 
                              value = c(-2.5,2.5), step = 0.1),
                  sliderInput('pfx_z', "Vertical Break", min = -2.5, max = 2.5, 
                              value = c(-2.5,2.5), step = 0.1),
                  ...
                  sliderInput('spin_eff', "Spin Efficiency", min = 0, max = 1, 
                              value = c(0,1.2), step = 0.01)
                ),
                box(
                  height = "800px",
                  id = "filter_sliders_2",
                  sliderInput('tunnel', "Tunnel Difference", min = 0, max = 6, 
                              value = c(0,6), step = 0.1),
                  sliderInput('release', "Release Difference", min = 0, max = 1, 
                              value = c(0,1), step = 0.1),
                  sliderInput('plate', "Plate Difference", min = 0, max = 6, 
                              value = c(0,6), step = 0.1),
                  sliderInput('diffspeed', "Speed Difference", min = -35, max = 0, 
                              value = c(-35,0)),
                  sliderInput('PTB', "Post Tunnel Break", min = 0, max = 1.5, 
                              value = c(0,1.5), step = 0.05),
